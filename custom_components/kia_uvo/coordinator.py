...
class HyundaiKiaConnectDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def _load_stored_token(self):
        """Load the stored token from config entry data."""
    ...
    def _save_token(self):
        ...
        if self.vehicle_manager.token:
            token_data = {
                "username": self.vehicle_manager.token.username,
            ...
            _LOGGER.debug(f"{DOMAIN} - Saved token to config entry")
    ...
    async def async_check_and_refresh_token(self) -> bool:
        """Refresh token if needed via library. Returns True if token was refreshed."""
        try:
