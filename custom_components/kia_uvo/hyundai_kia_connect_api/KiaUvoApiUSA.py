...
class KiaUvoApiUSA(ApiImpl):
    ...
    def __init__(self, region: int, brand: int, language) -> None:
        ...
        # Generate a proper UUID format like the iOS app uses
    ...
    def _send_otp(self, otp_key: str, notify_type: str, xid: str) -> dict:
            ...
            # Convert PHONE to SMS as API expects SMS
            if notify_type.upper() == "PHONE":
                notify_type = "SMS"
    ...
    def start_login(
        ...
        # Check for error 9789 and retry without rmtoken if needed
        if response_json.get("status", {}).get("statusCode") != 0:
            ...
            # If error 9789 and we had rmtoken, try without it
        ...
        # If we still get an error after retry, raise it
        status_code = response_json.get("status", {}).get("statusCode", 0)
        if status_code != 0:
            error_msg = response_json.get("status", {}).get("errorMessage", "Unknown error")
            raise AuthenticationError(
                f"{DOMAIN} - Login failed. Error {response_json.get('status', {}).get('errorCode')}: {error_msg}"
            )
    ...
    def login(
        ...
        if "payload" in response_json and "otpKey" in response_json["payload"]:
            ...
            # If we're being called from coordinator without an OTP handler, we can't continue
            handler = otp_handler or getattr(self, "_otp_handler", None)
            if not handler:
                _LOGGER.error(f"{DOMAIN} - OTP required but no handler available - need reauth")
                raise AuthenticationError(f"{DOMAIN} - OTP authentication required - please reconfigure integration")
            ...
            try:
                ...
                _LOGGER.debug(f"{DOMAIN} - otp_handler choose_destination failed")
